...
#topControls {position: fixed; top: 20px; right: 20px; display: flex; gap: 4px; z-index: 10; align-items: center;}
#topControls button {width: 28px; height: 28px; border-radius: 4px; border: 1px solid var(--border-main); background: var(--bg-panel); color: var(--text-main); cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 4px; box-sizing: border-box;}

.kql-icon {width:14px; height:14px; border-radius:50%; display:inline-block;}
.kql-off-icon {background:#ffffff; box-shadow:0 0 2px rgba(255,255,255,0.6);}
.kql-on-icon {background: conic-gradient(#00c853,#458bc5,#e07b7b,#d19a66,#bb86fc,#00c853); box-shadow:0 0 4px rgba(255,255,255,0.2);}
...
#filterInputBox {width: 100%; box-sizing: border-box; padding: 10px; font-size: 10px;}
...
thead, th {position: sticky; top: 0; z-index: 2; background: var(--bg-header); font-weight: bold;}
...
.kql-comment {color:#00c853 !important; background:rgba(0,200,83,0.08); padding:0 2px; border-radius:3px; font-weight:bold;}
.kql-table {color:#e07b7b; font-weight:bold;}
.kql-filters {color:#559955; font-weight:bold;}
.kql-structure {color:#458bc5;}
.kql-data {color:#d19a66;}
.kql-pipe {color:#cc0000; font-weight:bold;}
.kql-brackets {color:#ffd866;}
.kql-operators {color:#00ddff; font-weight:bold;}
 body.kql-disabled .kql-structure, body.kql-disabled .kql-table, body.kql-disabled .kql-filters, body.kql-disabled .kql-data, body.kql-disabled .kql-pipe, body.kql-disabled .kql-brackets, body.kql-disabled .kql-operators {color:#eeeeee !important; font-weight: normal !important;}

td.query pre {cursor: pointer; position: relative; line-height: 1.35em; max-height: calc(1.35em * 30); overflow-y: auto; background-color:#151515; color:#eeeeee;}
...
<script>window.kqlTheme = {comment: "{{COMMENT_COLOR}}", commentBg: "{{COMMENT_BG}}", table: "{{TABLE_COLOR}}", structure: "{{STRUCTURE_COLOR}}", func: "{{FUNCTION_COLOR}}", disabled: "#eeeeee"};</script>
</head>

<body>
<h1 style="margin-bottom:10px;">Azure Sentinel Analytics Rules</h1>
<span class="stat-muted" style="font-size:12px; margin-left:10px;">Snapshot taken: {{SNAPSHOTDATE}} (created by AllKQLtoHTML v{{VERSION}})</span>

<div id="topControls"><button id="kqlToggle" title="Toggle KQL context"><span id="kqlIcon" class="kql-icon"></span></button><button id="themeToggle" title="Toggle light/dark mode">🌙</button></div>
...
<div id="tocContent" class="toc"><ul>{{TOC}}</ul></div></div><br>
...
/* CONTEXT TOGGLE */
let kqlEnabled = true;
function updateKqlIcon() {const icon = document.getElementById('kqlIcon'); icon.classList.remove('kql-on-icon', 'kql-off-icon');
if (kqlEnabled) {icon.classList.add('kql-on-icon');}
else {icon.classList.add('kql-off-icon');}}
function toggleKqlContext() {kqlEnabled = !kqlEnabled; document.body.classList.toggle('kql-disabled', !kqlEnabled); updateKqlIcon();}
document.getElementById('kqlToggle').addEventListener('click', toggleKqlContext);
updateKqlIcon();
...
/* CTRL+Z TO CLEAR FILTERS */
document.addEventListener('keydown', function (e) {if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z') {if (document.activeElement && (document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'TEXTAREA')) {return;}
e.preventDefault(); activeFilters.clear(); includeFilter = null; excludeFilter = null; reverseMode = false; toggles.forEach(t => t.classList.remove('active')); 
...
document.addEventListener('keydown', function (e) {if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'x') {e.preventDefault();
...
/* MITRE ATT&CK NAVIGATOR FILE TO CLIPBOARD */
...
/* BUILD MITRE DESCRIPTIONS FOR MARKDOWN CREATION */
const mitreRaw = `{{MITREBLOCK}}` || ""; const mitreByRule = {};
if (mitreRaw) {mitreRaw.split('\n').forEach(line => {const parts = line.split('|');
if (parts.length < 5) return;
const rule = parts[0]; const tactic = parts[1]; const techId = parts[2]; const name = parts[3]; const desc = parts.slice(4).join('|');
if (!mitreByRule[rule]) mitreByRule[rule] = []; mitreByRule[rule].push({ tactic, techId, name, desc });});}

/* RULE MARKDOWN CREATION */
function buildMarkdown(rule) {const p = rule.properties || {}; //const ruleMitre = mitreByRule[rule.name] || mitreByRule[rule.displayName] || mitreByRule[rule.properties?.displayName] || [];
function val(v) {if (v === null || v === undefined) return "";
if (Array.isArray(v)) return v.join(', ');
if (typeof v === 'object') return JSON.stringify(v);
return String(v);}

function formatObjectList(v) {if (!Array.isArray(v)) return formatObject(v);
return v.map(item => formatObject(item)).join(",\n");}

function formatObject(v) {if (v === null || v === undefined) return "";
if (Array.isArray(v)) {return v.map(formatObject).join(", ");}
if (typeof v === "object") {return Object.entries(v)
.map(([k, val]) => {if (Array.isArray(val)) {return `${k}=[${val.map(formatObject).join(", ")}]`;}
if (typeof val === "object") {return `${k}=${formatObject(val)}`;}
return `${k}=${val}`;})
.join("; ");}
return String(v);}

function mitre(v) {const text = val(v);
return text.replace(/\b(T\d{4})(?:\.(\d{3}))?\b/g, (m, technique, sub) => {const url = sub
? 'https://attack.mitre.org/techniques/' + technique + '/' + sub
...
const kql = val(p.query); const today = new Date().toLocaleDateString('en-US');

const mitreRefs = [];
(p.techniques || []).forEach(t => {mitreRefs.push(`https://attack.mitre.org/techniques/${t.replace("T","")}`);});
(p.subTechniques || []).forEach(st => {if (!st) return;
const parts = st.split("."); mitreRefs.push(`https://attack.mitre.org/techniques/${parts[0].replace("T","")}/${parts[1]}`);});

const unique = [...new Set(mitreRefs)];
const parsed = unique.map(url => {const parts = url.split('/techniques/')[1]?.split('/') || []; const tech = parseInt(parts[0], 10) || 9999; const sub = parseInt(parts[1], 10) || 0;
return { url, tech, sub };});
parsed.sort((a, b) => {if (a.tech !== b.tech) return a.tech - b.tech; return a.sub - b.sub;});
const referencesBlock = parsed.map(x => `- ${x.url}`).join("\n");

const ruleKey = rule?.name;

const ruleMitreRaw = mitreByRule[ruleKey] || [];
const ruleMitre = [...ruleMitreRaw].sort((a, b) => {const aTech = parseInt((a.techId.match(/\d+/) || [9999])[0], 10); const bTech = parseInt((b.techId.match(/\d+/) || [9999])[0], 10); const aSub = (a.techId.split('.')[1] || 0); const bSub = (b.techId.split('.')[1] || 0);
if (aTech !== bTech) return aTech - bTech; return aSub - bSub;});

const playbook = ruleMitre.length
? ruleMitre.map(m => `- Tactics: ${m.tactic}\n[${m.techId}] ${m.name}\nDescription: ${m.desc}`).join('\n\n')
: 'None';

function sortMitre(v) {if (!Array.isArray(v)) return val(v);
return [...v]
.filter(Boolean)
.sort((a, b) => {
const aParts = String(a).match(/^T(\d{4})(?:\.(\d{3}))?$/i) || []; const bParts = String(b).match(/^T(\d{4})(?:\.(\d{3}))?$/i) || []; const aTech = parseInt(aParts[1] || '9999', 10); const bTech = parseInt(bParts[1] || '9999', 10); const aSub = parseInt(aParts[2] || '0', 10); const bSub = parseInt(bParts[2] || '0', 10);
if (aTech !== bTech) return aTech - bTech; return aSub - bSub;})
.join(', ');}
...
'**techniques :** ' + mitre(sortMitre(p.techniques)),
'**subTechniques :** ' + mitre(sortMitre(p.subTechniques)),
'**alertRuleTemplateName :** ' + val(p.alertRuleTemplateName),
'**incidentConfiguration :** ' + formatObjectList(p.incidentConfiguration),
'**eventGroupingSettings :** ' + formatObjectList(p.eventGroupingSettings),
'**alertDetailsOverride :** ' + formatObjectList(p.alertDetailsOverride),
'**customDetails :** ' + formatObjectList(p.customDetails),
'**sentinelEntitiesMappings :** ' + formatObjectList(p.sentinelEntitiesMappings),
'**entityMappings :** ' + formatObjectList(p.entityMappings),
'**id :** ' + val(p.id),
'**kind :** ' + rule.kind,'',
'* * *','',
'**Playbook:**','',
...
/* RULE MARKDOWN COPY TO CLIPBOARD */
